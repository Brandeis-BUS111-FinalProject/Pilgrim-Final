...
Pilgrim Bank's senior management is currently reconsidering bank's internet strategy -- whether to charge service fee for those customers using online channel or offer with lower service charge to engage customers. To make the decision, the key point is to answer if online customers could bring higher profit or secure a higher retention rate. In our report, we described the dataset, and further conducted regression and correlation test to see whether online customers could bring higher profit or have associated with higher retention rate. If the analysis shows online customers are indeed better customers, the senior management would decide to offer rebates or lower the service charges for customers using online banking. 
...
```
Split the data and only get a subset for 1999.
...
#consumerDB[consumerDB$X9Online==0&consumerDB$X9Billpay==1,]$status_2000 = NONE
...
#TransitionMatrix = cbind(TransitionMatrix,rowSums(TransitionMatrix))
TransitionMatrix
...
So 17% of those customers with no online account and no billpay in 1999 leave the bank in 2000; 14% of those customers with online account but no billpay in 1999 leave the bank in 2000; and 15% of those customers with both online account and billpay in 1999 leave the bank in 2000. Although there is no significant differences among these percentages, the small discrepancy shows that those who have no online account and no billpay have the higher possibility to leave the bank. Therefore, customers who use online accounts and billpay are more likely to stay with the bank.
...
summary(lm_profit)
```
A regression was calculated to see the the use of online banking and billpay has any effect on profit in year 1999.The difference between not using the online banking and using the online banking decrease by 6.619 unit on profit. The difference between not using the billpay and using the billpay increase by 91.240 unit on profit. When customers do not use online banking or billpay service, the estimated profit mean is 110.786. 
There is a significant regression equation was found (p= 0.001597 < 0.05).That is saying, there is significant difference on customer profitability between the use of the online banking&billpay and not using the online banking& billpay. R square is 0.001661, meaning that 0.16% of the variance in profit can be explain by using Online banking and billpay. Adjusted R-squared is 0.001597, which is used for testing a goodness-of-fit. Adjusted R-squared is a modified version of R-squared that has been adjusted for the number of predictors in this model. The adjusted R-squared increases only if the new term improves the model more than would be expected by chance. It decreases when a predictor improves the model by less than expected by chance.

```{r}
lm_retention = lm(retention ~ X9Online+X9Billpay, data = consumerDB)
summary(lm_retention)
```
A regression was calculated to see the the use of online channel and billpay has any effect on tenure length in year 1999.The difference between not using the online service and using the online service could increase the average tenure length by 0.028106. The difference between not using the billpay and using the billpay decrease by 0.011407 unit on tenure length. When customers do not use online banking service, the estimated tenure mean is 0.831785 years.
There is a significant regression equation was found (p= 0.0001403 < 0.05).That is saying, there is significant difference on retention bewtween the use of online banking& billpay and not using online banking& billpay. R square is 0.0005608, meaning that 0.05% of the retention can be explain by using Online banking. Adjusted R-squared is 0.0004976, which is used for testing a goodness-of-fit. Adjusted R-squared is a modified version of R-squared that has been adjusted for the number of predictors in this model. The adjusted R-squared increases only if the new term improves the model more than would be expected by chance. It decreases when a predictor improves the model by less than expected by chance.
...
We first create a base model to compare the relationship bwteen online uses and profit. Expected 1999Profit = 110.786 + 5.881*Online. Both intercept of the regression (beta0) and coeffcient on Online (beta 1) are statistically significant. Beta 1 represents that when a customer starts to use online banking, profit will increse by 5.881. The R-square is  1.809e-05, which means that a really small portion of profit can be explained by the variation of x variable.

Finally, we add 1999District into the regression and create corresponding mixed variables. The intercept, 1999Age, Income level, and tenure are significant. For base case, when a customer starts to use online banking, the expected 1999Profit will decrease by 14.79437, holding other variable constant. And when a customer starts to use billpay service, the expected 1999Profit will increase by 42.73223, holding other variable constant. Then if we hold other constant but increase the age category to the second category, when customer start to use online banking, the expected 1999Profit will increase 18.01218. Among thoese statistically insignificant coeffcient, we will interpret the significant one. For example, factor(X9Inc)2:X9Online =-3.18762. It means it is the profit difference of a customer in Income Level 2 and uses online banking, eliminating other effects on age, distrct, billpay, or tenure.  
The R-square is 0.06888, which means that 6.888% of 1999Profit can be explained by the variations in x variables.

### (4) Major Statistics Summary
ConsumerDB2 is a dataset that only contains 26396 observations and 14 variables. It's a subset from the orginal dataset. consumerDB2 only contains those observations that don't have any missing data in 2000 Online, 2000 profit and 2000 billpay.
```{r}
...
test2 = consumerDB2 [!is.na(consumerDB$X9Inc) & !is.na(consumerDB$X9Age)]
```
...
```
We first created a subset by randomly choosing 5% from the orginal 1999 year's datapoints. This is used as the validation for the following model we created. 5% of the data will be the test dataset, while the 90% of the data will be the training dataset.
...
```
We calculated the predicted errors for all profit models, and the prediction error for our final Profit Model is 269.9520. To compare the errors from more perspectives, we made the following table.
...
comparison.table.profit = matrix(c(summary(lm_profit_Online_Billpay)$adj.r.square, 
            ...
            BIC(lm_profit_final), prediction.error5), nrow = 5, ncol = 4, byrow = TRUE)
...
lm_retention_final = lm(retention ~ factor(X9Age) + factor(X9Inc) 
                      + X9Online * factor(X9Age) + X9Online * factor(X9Inc) 
                      + X9Billpay * factor(X9Age) + X9Billpay * factor(X9Inc) 
                      + X9Tenure * factor(X9Age) + X9Tenure * factor(X9Inc), data = consumerDB)
...
At the base case, we compare the relationship between Online and retention. Expected 1999Retention =  0.831785 + (0.026544)*Online. If the customers use the online banking, the retention will increase by (0.026544). The intercept of the regression (beta0) is statistically significant. The R-square is 0.0005156, which means that 0.52% of retention can be explained by the variation in Online banking in 1999.

Finally, we add Tenure into the regression. The intercept, 1999 Age, Income level are significant. For base case, when a customer starts to use online banking, the expected retension will decrease by 0.0560975, holding other variable constant. And when a customer starts to use billpay service, the expected 1999Profit will decrease by 0.1335309, holding other variable constant. The coeffcient of factor(X9Inc)6:X9Online (0.1132973) is the difference of the retention between groupA (income level1& Age level1& use online service) the groupB (income level6 & Age level1& use online service), eliminating changes in other factors. The R-square is 0.1199, which means that 11.99% of Profit can be explained by the variations in Online banking, Age bucket, Income level bucket, online service, billpay service and Tenure.
...
predicted.retention4 = predict(lm_retention_final, validation.data)
prediction.error.retention4 = sqrt(mean((predicted.retention4-validation.data$retention)^2))

```
We calculated the predicted errors for all retention models, and the prediction error for our final retention Model is 0.188004 To compare the errors from more perspectives, we made the following table.
...
comparison.table.retention = matrix(c(summary(lm_retention_Online_Billpay)$adj.r.square, 
              ...
              BIC(lm_retention_final), prediction.error.retention4), nrow =4, 
              ncol = 4, byrow = TRUE)
...
rownames(comparison.table.retention) = c("lm_retention_Online", "lm_retention_Age", 
         "lm_retention_Inc", "lm_retention_final")
comparison.table.retention
```
We compared the adjusted R-square, AIC, BIC, and Prediction Error. It shows that the first Online Retention Model has the lowest AIC of 23098.44, lowest BIC of 23608.52, and lowest Prediction Error of 0.188004. These are statistics that we can use to test the goodness of fit. The lower the AIC, BIC, and Prediction Error, the better the model. The final model has the highest adjust R square, implying that we chose the numbers of predictors properply. Therefore, we condluded that the lm_retention_final Model best fits the validation set.
...
round(table(hit_rate$hit_times)/sum(table(hit_rate$hit_times)),2)
```
Since the our retention prediction model offers the posibility of a customer's status. If the posibility is greater than .5, we identify this customers as they will stay with the bank; while posibility smaller than .5 as this customer will leave the bank. Then we compare this to the actual data in 2000 year, and we found out that we made the correct prediction for 84% of the dataset. 
...
According to the result, the prediction error of our Profit Model is 382.7333 Meanwhile, 84% of the prediction is correct for our Retention Model, which is pretty accurate. In general, our Profit Model and Retention Model could fairly predict the profit and retention status of Pilgrim Banks' customers.
...
```
In order to estimate the total profit range, we run our Profit Model and extracted 95% of the dataset's profit of the lower and upper limit. After we added all the predicted upper limited, we got the highest total profit of 179295.5; and in the same way, we got the lowest total profit of 128721.6. So the range of 2000's profit is from 128721.6 to 179295.5
